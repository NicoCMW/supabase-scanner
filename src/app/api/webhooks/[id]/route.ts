...
/**
 * GET /api/webhooks/:id
 * Get a single webhook including its signing secret.
 */
export async function GET(
  ...
  const { data, error } = await supabase
    .from("webhooks")
    .select("id, project_id, label, url, secret, enabled, notify_scan_complete, notify_critical_finding, notify_score_degradation, created_at, updated_at")
    .eq("id", id)
    .eq("user_id", user.id)
    .single();
